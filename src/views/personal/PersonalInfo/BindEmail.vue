    ...
    <div class="center-wrap bindEmail">
      <form v-if='!successbind'>
        ...
        <div class="email-box email-btn">
          ...
          <button type="button" @click="bindEmailFun" class="findEmail btnm btn-confirm">{{$t('button.confirm')}}</button>
          <!-- <button class="findEmail btnm btn-cacel ">{{$t('button.cancel')}}</button> -->
        </div>
      </form>
      <!--完成验证后样式-->
      <div class="successBind" style="margin:50px;" v-if='successbind'>
        <p>
          {{$t('editProfile.bindedEmail')}}
          <span style="font-weight: bold;margin: 0 20px 0 10px;">{{bindForm.email}}</span>
          <button type="button" @click='SbindEmail' class="btn btn-primary" style="background-color:#1e8fff;">{{$t('button.edit')}}</button>
        </p>
      </div>
      <!--发邮件模态框-->
