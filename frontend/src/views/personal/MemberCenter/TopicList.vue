...
export default {
  ...
  computed: {
    ...
    showPageBtn() {
      ...
      if (this.cpno >= this.cpageCount - 1)
        return [
          1,
          "···",
          this.cpageCount - 2,
      ...
      if (this.cpno === this.cpageCount - 2)
        return [
          1,
          "···",
          this.cpageCount - 3,
  ...
  methods: {
    ...
    loadTopicPage() {
      if (this.tpno < this.pageCount) {
        setTimeout(() => {
          this.showLoader = true;
          this.tpno++;
          get(`/api/u/topic/${this.userInfo.username}/${this.tpno}`).then(
            data => {
    ...
    showDiscuss(index, id) {
      ...
      $(".editor-toolbar")
        .find(".wangeditor>.report")
        .css({ bottom: "0" });
      $(".editor-toolbar")
        .find(".wangeditor>.cancel")
        .css({ bottom: "4px" });
      $(".editor-toolbar")
        .find(".wangeditor>.editor")
        .css({ "min-height": "130px", "padding-bottom": "37px" });
      $(".editor-toolbar")
        .find(".wangeditor>div:eq(2)")
        .css("display", "none");
    },
    // 是否显示评论默认框
    commentShowFun() {
    ...
    EditorContent(val) {
      ...
      let now = val.replace(
        /<p[^>]*>|<\/p>|<span[^>]*>|<\/span>|<br>|<h[1-6][^>]*>|<\/h[1-6]>|<h-char[^>]*>|<img[^>]*>|<\/h-char>|<h-inner>|<\/h-inner>/g,
        ""
      );
      if (now.indexOf("&nbsp;") > 0) {
        now = now.replace(/&nbsp;*/g, "");
    ...
    replyFun(val) {
      ...
      let reply = val.replace(
        /<p[^>]*>|<\/p>|<h-char[^>]*>|<\/h-char>|<h-inner>|<\/h-inner>/g,
        ""
      );
      ...
      } else if (/^\/static.*/gi.test(reply)) {
        return (
          "图片评论" +
          `<a style='color:#0181FF' href='${reply}'><i class='iconfont'>&#xe694;</i>查看图片</a>`
        );
    ...
    sortList(id, tmpId) {
      ...
      if (id === 0) {
        ...
        get(`/api/topic/${tmpId}/1`).then(data => {
          if (!this.nowData[tmpId])
            this.$set(this.nowData, tmpId, data.data.replies);
