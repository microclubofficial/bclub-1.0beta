            ...
            <div class="clearfix edit-infor">
              ...
              <div class="left left-width">
                ...
                <p class="form-control-static prompt" style="margin-top:0px !important;">{{unamePrompt}}</p>
                  <div class="clearfix">
                    <button type="button" class="edit-user-btn" v-bind:disabled="!setForm.username" @click="setusername(0)"  >{{$t('button.confirm')}}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div class="left left-width">
                ...
                <div class="buttons-box clearfix">
                  <div class="left left-width">
                    ...
                    <button type="button" class=" edit-user-btn" v-bind:disabled="!setForm.phone" @click="setusername(1)"  >{{$t('button.confirm')}}
                    </button>
                  </div>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    getPhoneControl () {
      ...
      this.hasphone = true
      let that = this
      post('/api/phoneCaptcha', { 'phone': this.setForm.phone }).then((data) => {
        if (data.resultcode === 0) {
          if (data.message === 'failed' || data.message === '失败') {
            alert(this.$t('prompt.phoneRegistered'))
            this.hasphone = true
            return false
          } else if (data.message === 'Wrong format of phone number' || data.message === '手机号码格式错误') {
