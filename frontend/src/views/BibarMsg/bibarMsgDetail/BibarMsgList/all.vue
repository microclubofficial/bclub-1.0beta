...
export default{
  ...
  methods: {
      ...
      this.$router.push({
        ...
        query: {
          a: JSON.stringify([
            ...
            {label: this.$t('breadcrumb.coinNews'), path: '/bibarLayout'},
    ...
    showContent (data) {
      ...
      let articlesNum = parseInt(this.bibarArticles[this.i].replies_count) / 5 - parseInt(parseInt(this.bibarArticles[this.i].replies_count) / 5)
      let id = this.bibarArticles[this.i].id
      ...
      if (this.isdelTopic) {
        ...
        post(`/api/reply/delete/${this.delItem}`).then(data => {
          if (data.resultcode === 1) {
            ...
            let id = this.bibarArticles[this.i].id
            if (parseInt(this.bibarArticles[this.i].replies_count) % 5 === 0) {
              if (this.cpno[id] > 1) {
